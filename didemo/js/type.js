...
$(function() {
$(document).ready(function(){
	var netName = window.didemo.networkInfo();
var storeName = window.didemo.storeName();
$(".wifi_pic span").html(netName);
$(".mncs_pic span").html(storeName);
	
  $("#boxpic1").focus(function(){
    $("#boxpic1").css("box-shadow","0px 0px 18px #e5e4f3");
  });
  $("#boxpic1").blur(function(){
    ...
    $("#boxpic2").css("box-shadow","0px 0px 18px #e5e4f3");
  });
...
//815
	...
	$(".tj_wrapper").scroll(function(){
		var a=$(this).scrollTop() + parseInt($(this).css("height").replace("px",""));
		var b=document.getElementById("tj").scrollHeight;
		var num= b-a;
		console.log((b-a));
		if($(this).scrollTop() == 0){
...
function layload(){
	$(".tj_wrapper").css('height',(parseInt($('.boxpic').height()*3) + parseInt(30)));
	$(".tj_wrapper").css("overflow-y","scroll");
}
